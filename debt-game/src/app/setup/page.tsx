  ...
  return (
    <div className="pt-6 space-y-4 pb-8">
      ...
      <div className="glass-accent p-6 text-center">
        ...
        <h1 className="text-2xl font-extrabold glow-gold" style={{ color: '#e8b849' }}>借金キラー</h1>
        <p className="text-xs mt-1.5 font-medium" style={{ color: '#8890b0' }}>借金討伐クエスト — 冒険の準備</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="glass p-5">
          <div className="section-bar" style={{ marginBottom: '14px' }}>
            <h2 className="text-[15px] font-extrabold" style={{ color: '#eef0f6' }}>ハンター情報</h2>
          </div>
          <div className="space-y-3">
            <div>
              <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>ハンター名</label>
              <input type="text" value={playerName} onChange={e => setPlayerName(e.target.value)} placeholder="ハンター" className="input-glass" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>月収（手取り）</label>
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>固定費（家賃等）</label>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="section-bar" style={{ marginBottom: 0 }}>
              <h2 className="text-[15px] font-extrabold" style={{ color: '#eef0f6' }}>討伐クエスト登録</h2>
            </div>
            ...
            {debts.map((debt, i) => (
              <div key={i} className="glass p-5">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-sm font-extrabold" style={{ color: '#e8b849' }}>クエスト #{i + 1}</span>
                  {debts.length > 1 && (
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>モンスター種別</label>
                    <select value={debt.debtType} onChange={e => updateDebt(i, 'debtType', e.target.value)} className="select-glass">
                      {DEBT_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>モンスター名（任意）</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>借金額（円）</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>年利（%）</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>毎月返済額</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: '#8890b0' }}>支払日</label>
      ...
      <div className="glass-inner p-5">
        <p className="text-xs font-bold mb-1" style={{ color: '#e85d5d' }}>⚠ データリセット</p>
        <p className="text-[10px] mb-3 font-medium" style={{ color: '#505878' }}>全てのデータを削除して最初からやり直します。</p>
